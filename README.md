...
My research focuses on the application of Bayesian multilevel, spatio-temporal modelling in Global Health and epidemiology, specifically for modelling the HIV epidemic in sub-Saharan Africa (even more specifically, in modelling [male circumcision coverage](https://arxiv.org/pdf/2108.09142.pdf) there (for which I am actively developing the R package [`threemc`](https://github.com/mrc-ide/threemc))). 

I am proficient in R (mainly using the `tidyverse`, `data.table` and `sf` for data manipulation, [`orderly`](https://github.com/vimc/orderly) for reproducible reporting, and `TMB`, `INLA` and `rstan` for Bayesian inference) and also have a limited knowledge of Python (mostly `numpy` and `skikitlearn` for machine learning) and C++. 
